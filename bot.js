...
let wss = "wss://yournodelink";
const secretKey = "YOURPRIVATEKEY"
const publicKey = "YOURADDRESS"
const web3 = new Web3(wss)


const TRACKED_CONTRACT = "0xb52575ce47c4534f7b0ba88acfba7546350197d8"
const BUY_THRESHOLD = 0.2

const PAN_ROUTER_ADDRESS =  "0x10ED43C718714eb63d5aA57B78B54704E256024E";
const BNB_CONTRACT = "TARGET TOKEN"
...
function calculate_gas_price(action, amount){
  if (action==="buy"){
	...
	try{
		parsedGas = ethers.utils.parseUnits(amount.add(1000000000).toString(), 'wei')
		//console.log("Gas: ", parsedGas.toString())
		return ethers.BigNumber.from(parsedGas.toString())
	}
	catch (error){
		console.log("couldnt convert ")
	}
  }else{
    //const gazLimit = ethers.utils.formatUnits(amount.sub(1000000000), 'wei')
	parsedGas = ethers.utils.parseUnits(amount.sub(1000000000).toString(), 'wei')
	//console.log("Gas: ", parsedGas.toString())
    return ethers.BigNumber.from(parsedGas.toString())
  }
}
...
const sellToken = async(account,tokenContract,gasLimit,gasPrice,value=99)=>{
  ...
  //const approve = await sellTokenContract.approve(PAN_ROUTER_ADDRESS, amountIn*100)
  ...
  //if (receipt_approve && receipt_approve.blockNumber && receipt_approve.status === 1) {
	...
	{
		//console.log(`Couldn't Sell! ${ex}`)
		sellToken(account, tokenContract, gasLimit, gasPrice, value)
	}
  //}
}
...
'function swapExactETHForTokensSupportingFeeOnTransferTokens(uint amountOutMin,address[] calldata path,address to,uint deadline)'])
  ...
  const trackedContract = new ethers.Contract(TRACKED_CONTRACT,swapAbi,account)
  
  const allowance = await trackedContract.allowance(publicKey, PAN_ROUTER_ADDRESS)
  
  if (allowance <= 0)
  {	
	 console.log("low allowance!")
	  const approve = await trackedContract.approve(PAN_ROUTER_ADDRESS, (await trackedContract.totalSupply()))
	  const receipt_approve = await approve.wait();
	  console.log(`Approved https://bscscan.com/tx/${receipt_approve.transactionHash}`);
  }
...
customWsProvider.on("pending", (tx) => {
  ...
  if(transaction && transaction.to && transaction.to.toLowerCase()==="0x10ED43C718714eb63d5aA57B78B54704E256024E".toLowerCase()){
	if (transaction && transaction.from && transaction.from.toLowerCase() === publicKey.toLowerCase()){
		console.log("found transaction from my address lol")
		return;
	}
    const value = web3.fromWei(transaction.value.toString())
    const gasPrice= web3.fromWei(transaction.gasPrice.toString())
    let gasLimit= web3.fromWei(transaction.gasLimit.toString())
    ...
    if(value>=BUY_THRESHOLD) {
      ...
      if(result.length>0){
        ...
        if(result[1].length>0){
		  ...
		  if (tokenAddress.toLowerCase() == TRACKED_CONTRACT.toLowerCase())
		  {
			  ...
			  console.log("Found Trade on ",tokenAddress," for ", value, " BNB")
			  ...
			  await Promise.all([buyToken(account,tokenAddress,transaction.gasLimit,buyGasPrice), sellToken(account,tokenAddress,transaction.gasLimit,sellGasPrice)]);
